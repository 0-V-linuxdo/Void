...
`);
  ...
  async function fetchBotUsage() {
    try {
      ...
      logger26.warn("Failed to fetch Grok Bot usage", error);
      return null;
    }
  }
  ...
  var usageDisplay_default = definePlugin({
    ...
    start() {
      ...
      try {
        ...
        logger26.warn("RoutingStore subscribe failed", error);
      }
    },
    stop() {
      unsubRoute?.();
...
`);
  ...
  var logger28 = new Logger("BetterFiles");
  ...
  async function deleteAssets(ids) {
    ...
    for (const id of ids) {
      ...
      try {
        ...
        logger28.error("Failed to delete asset", id, e);
      }
...
`);
  ...
  function currentChatIds() {
    ...
    try {
      ...
      logger29.debug("route chat ids unavailable:", e);
    }
    try {
      ...
      logger29.debug("url chat ids unavailable:", e);
    }
    return ids;
  }
  function currentAgentIds() {
    ...
    try {
      ...
      logger29.debug("route agent id unavailable:", e);
    }
    try {
      addId(ids, new URL(location.href).pathname.match(PATH_BOT)?.[1]);
    } catch (e) {
      logger29.debug("url agent id unavailable:", e);
    }
    return ids;
  }
  function currentIds() {
  ...
  function botLiveIds(ids) {
    try {
      ...
      logger29.debug("bots store unavailable:", e);
    }
  }
  function liveIds() {
    const ids = new Set;
  ...
  function errorOf(id) {
    ...
    try {
      ...
      logger29.debug("bot error lookup failed:", e);
    }
    return false;
  }
  ...
  function onStreamEnd5({ responseId }) {
    ...
    try {
      ...
      logger29.debug("streamEnd failed:", e);
      marks.set(cid, "done");
    }
    schedule();
  }
  ...
  var chatListStatus_default = definePlugin({
    ...
    start() {
      ...
      try {
        ...
        logger29.debug("bots store subscribe failed:", e);
      }
      observe();
      schedule();
    },
...
`);
  ...
  var logger30 = new Logger("CompactModeSelect");
  ...
  async function harvestIcons() {
    ...
    try {
      ...
      logger30.warn("Failed to harvest mode icons:", e);
    } finally {
      setPicking(false);
      harvesting = false;
    }
  ...
  async function selectMode(id) {
    ...
    try {
      ...
      if (!menu) {
        const trigger = nativeTrigger();
        if (!trigger) {
          logger30.warn("Native mode selector not found");
          return;
        }
        clickEl(trigger);
        menu = await waitForMenu();
      ...
      if (!menu) {
        logger30.warn("Native mode item not found:", id);
        return;
      }
      cloak(menu);
      stashGlyphs(menu.items);
      ...
      logger30.warn("Failed to select mode:", e);
    } finally {
      setPicking(false);
    }
  }
