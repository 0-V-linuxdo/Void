...
/**
 * Void++ [20260830.1] v1.0.0 — A modification for grok.com
...
`);
  ...
  // void-css:/tmp/void-src/src/plugins/pluginsFlyout/styles.css
...
`);
  ...
  function VersionInfo() {
    return /* @__PURE__ */ React.createElement(Flex, {
      ...
      as: "span",
      color: "secondary"
    }, "[20260830.1] v1.0.0"), /* @__PURE__ */ React.createElement(Dot, null), /* @__PURE__ */ React.createElement(VersionLink, {
...
`);
  ...
  var cl17 = classNameFactory("void-sf-");
...
`);
  ...
  var logger19 = new Logger("RecentTopics");
  var cl18 = classNameFactory("void-rt-");
  var HOME_KEY = "home";
  var HOME_SEP = "home:";
  ...
  function idsFromHistory() {
    try {
      ...
      logger19.debug("historyStack unavailable:", e);
      return [];
    }
  }
  function pageTitle() {
  ...
  function convWorkspaceId(id) {
    try {
      ...
      logger19.debug("convWorkspaceId failed:", e);
  ...
  function dropWorkspace(id) {
    const prev = settings6.plain.workspaceByConv ?? {};
    if (!prev[id])
      return;
    const next = { ...prev };
    delete next[id];
    settings6.store.workspaceByConv = next;
  }
  function workspaceOf(id) {
    ...
    if (isHomeId(id)) {
      ...
      if (fromKey)
        return fromKey;
      return id === currentVisit() ? liveWorkspaceId() : asWorkspaceId(settings6.plain.workspaceByConv?.[id]);
    }
    ...
    if (fromSidebar)
      return fromSidebar;
    const cached = asWorkspaceId(settings6.plain.workspaceByConv?.[id]);
    if (cached)
      return cached;
  ...
  function projectNameOf(id) {
    ...
    if (!ws)
      return "";
    const idx = sidebarIndex();
    const named = usableName(idx.nameByConv[id] || idx.nameByWs[ws] || wsNames[ws] || settings6.plain.projectNames?.[ws] || "");
    if (!named)
      return "";
    const live = liveWorkspaceId();
  ...
  function rememberProject(id) {
    ...
    const name = usableName(sidebarName || wsNames[ws] || liveName || settings6.plain.projectNames?.[ws] || "");
    if (!name)
      return;
    wsNames[ws] = name;
  ...
  function reconcileSidebarCache() {
    ...
    if (wsChanged)
      settings6.store.workspaceByConv = prevWs;
    if (namesChanged)
      settings6.store.projectNames = prevNames;
  }
  function requestWorkspace(id) {
    ...
    try {
      ...
      fetchConv(id).then((conv) => {
        ...
        const prev = settings6.plain.workspaceByConv ?? {};
        if (prev[id] !== ws)
          settings6.store.workspaceByConv = { ...prev, [id]: ws };
        const live = liveWorkspaceId();
        const liveName = usableName(readOpenProjectName());
        const names = settings6.plain.projectNames ?? {};
        if (live && ws !== live && liveName && names[ws] === liveName) {
          const next = { ...names };
          delete next[ws];
          settings6.store.projectNames = next;
          delete wsNames[ws];
        }
        if (open2)
          paint();
      }).catch((e) => logger19.debug("workspace fetch failed:", e)).finally(() => {
        pendingWs.delete(id);
      });
    } catch {
      pendingWs.delete(id);
  ...
  function linesFromStore(id) {
    ...
    try {
      ...
      logger19.debug("ResponseStore snapshot failed:", e);
      return [];
    }
  }
  ...
  function snapOf(id) {
    return thumbs.get(id) ?? parseSnap(settings6.plain.pages?.[id]);
  }
  function rememberPage(id, snap) {
    const json = JSON.stringify(snap);
    const prev = settings6.plain.pages ?? {};
    if (prev[id] === json)
      return;
    settings6.store.pages = { ...prev, [id]: json };
  }
  ...
  function buildPageShot(snap) {
    const page = node("span", cl18("page"));
    page.dataset.theme = snap.theme;
    for (const line of lastRound(snap.lines)) {
      const el = node("span", cl18("page-line", line.role === "user" && "page-line-user"), line.text);
  ...
  function captureCurrent() {
    ...
    try {
      ...
      logger19.debug("snapshot failed:", e);
    } finally {
      capturing = false;
    }
  }
  ...
  function bump(id) {
    if (!id)
      return;
    if (isHomeId(id) && !settings6.store.includeHome)
      return;
  ...
  function parseHref(href) {
    try {
      ...
      logger19.debug("urlToRoute failed:", e);
    }
    return null;
  }
      ...
      if (dest.page !== "workspace") {
        try {
          ...
          fetchConv?.(id).then((conv) => {
            ...
            applyChatPage(id, ws);
            rememberProject(id);
          }).catch((e) => logger19.debug("workspace resolve failed:", e));
        } catch (e) {
          logger19.debug("workspace fetch skipped:", e);
        }
      }
    } catch (e) {
  ...
  function begin(reverse, fromHold) {
    ...
    try {
      ...
      logger19.error("Failed to open switcher:", e);
    }
    paint();
  }
  ...
  function onKeyDown(e) {
    ...
    if (combo) {
      ...
      try {
        ...
        logger19.error("Hotkey failed:", err);
      }
      return;
    }
    if (!open2)
  ...
  function fillShot(box, id) {
    ...
    if (!snap) {
      const fallback = node("span", cl18("fallback"));
      fallback.append(faviconImg(cl18("favicon")));
      box.append(fallback);
      return;
    }
    const shot = node("span", cl18("shot"));
  ...
  function folderIcon() {
    ...
    svg2.setAttribute("class", cl18("folder"));
  ...
  function renderList(items) {
    if (!host)
      return;
    const panel = host.querySelector(`.${cl18("panel")}`);
    if (!panel)
      return;
    let list = panel.querySelector(`.${cl18("list")}`);
    if (!list) {
      panel.replaceChildren();
      list = node("div", cl18("list"));
      panel.append(list);
    }
    list.replaceChildren();
    items.forEach((topic, i) => {
      const btn = node("button", cl18("card"));
      btn.type = "button";
      btn.tabIndex = -1;
      ...
      const shot = node("span", cl18("thumb"));
      shot.setAttribute("aria-hidden", "true");
      fillShot(shot, topic.id);
      const meta = node("span", cl18("meta"));
      meta.append(node("span", cl18("name"), topic.title));
      if (topic.project) {
        const proj = node("span", cl18("host"));
        proj.append(folderIcon(), node("span", cl18("host-name"), topic.project));
        meta.append(proj);
      }
  ...
  function syncActive() {
    if (!host)
      return;
    const cards = host.querySelectorAll(`.${cl18("card")}`);
  ...
  function paint() {
    ...
    if (!items.length) {
      if (paintedKey !== "__empty__") {
        panel.replaceChildren(node("div", cl18("empty"), "Open a few chats, then hold Ctrl+` to switch."));
        paintedKey = "__empty__";
      }
      requestAnimationFrame(() => panel.setAttribute("data-visible", "true"));
      return;
    }
    if (paintedKey === "__empty__" || !panel.querySelector(`.${cl18("list")}`)) {
      panel.replaceChildren(node("div", cl18("list")));
      paintedKey = "";
    }
    if (paintedKey !== key) {
      renderList(items);
  ...
  var recentTopics_default = definePlugin({
    ...
    start() {
      ...
      try {
        ...
        logger19.error("Hydrate failed:", e);
      }
      if (!keys) {
    ...
    onSettingsChange() {
      try {
        ...
        logger19.error("Settings update failed:", e);
      }
    },
    zustand: {
      RoutingStore: {
...
`;
  ...
  function apply() {
    ...
    if (settings7.store.hideDictationRefinement)
...
`).map((s) => s.trim()).filter(Boolean);
  ...
  function PhrasesEditor() {
    const { phrases } = settings11.use(["phrases"]);
    return /* @__PURE__ */ React.createElement(Flex, {
      flexDirection: "column",
      gap: "0.5rem",
      className: cl22("root")
      ...
      onChange: (e) => {
        settings11.store.phrases = e.target.value;
      },
  ...
  var placeholder_default = definePlugin({
    ...
    settings: settings11,
    _phrases() {
      const lines = parsePhrases(settings11.store.phrases ?? DEFAULT_PHRASES);
...
`);
  ...
  var logger23 = new Logger("InputHistory");
...
`) : el.innerText ?? el.textContent ?? "";
  ...
  function placeCaret(el, atStart) {
    try {
      ...
      logger23.debug("placeCaret pm failed:", err);
    }
  ...
  function setEditorText(el, text, atStart) {
    ...
    try {
      ...
      logger23.debug("insertText failed:", err);
    }
...
`).length;
      ...
      return /* @__PURE__ */ React.createElement("div", {
        ...
        "aria-label": "Copy",
        onClick: () => {
          copyToClipboard(row.text).catch((err) => logger23.error("copy failed:", err));
        }
      }, /* @__PURE__ */ React.createElement(CopyIcon, {
        size: 18
      })), /* @__PURE__ */ React.createElement(ButtonWithTooltip, {
...
`);
  ...
  var logger25 = new Logger("UsageDisplay");
  ...
  function storeGet(key) {
    ...
    try {
      ...
      logger25.debug("Failed to read usage stats", error);
      return memory.get(key) ?? null;
    }
  }
  function storeSet(key, value) {
    ...
    try {
      ...
      logger25.debug("Failed to persist usage stats", error);
      memory.set(key, value);
    }
  }
  function storeRemove(key) {
    ...
    try {
      ...
      logger25.debug("Failed to clear usage stats", error);
      memory.delete(key);
    }
  }
  ...
  function snapshotToday() {
    if (!settings17.store.usageStats)
      return;
    syncAccount();
    if (!state.userId)
      return;
    recordSnapshot(state.userId, state.usage?.weekly.usedPercent ?? null, state.usage?.weekly.resetAt ?? null, retainDaysOf(settings17.store.retainDays));
  }
    ...
    refreshPromise = (async () => {
      try {
        ...
        const remote = await fetchOfficialUsage().then((usage) => ({ ok: true, usage })).catch((error) => {
          logger26.warn("Failed to fetch official usage", error);
          return { ok: false };
        });
  ...
  function ButtonIcon() {
    ...
    const { showPercent } = settings17.use(["showPercent"]);
  ...
  function UsagePanel() {
    useExternalStore(store3);
    const { usageStats, hoverStatsDelay } = settings17.use(["usageStats", "hoverStatsDelay"]);
  ...
  function StatsToggle() {
    const { usageStats } = settings17.use(["usageStats"]);
    return /* @__PURE__ */ React.createElement(Flex, {
      alignItems: "center",
      justifyContent: "space-between",
      gap: "0.75rem",
      ...
      onCheckedChange: (value) => {
        settings17.store.usageStats = value;
        store3.notify();
        if (value)
          refresh("manual");
      }
...
`;
  ...
  function getColor(key, fallback) {
    const val = settings21.store[key];
    return val && isValidHex(val) ? val : fallback;
  }
  function applyColors() {
    const customLink = settings21.store.linkColor;
    ...
    if (settings21.store.enableVisitedColor) {
      const customVisited = settings21.store.visitedColor;
  ...
  function ColorRow2({ settingKey, title, description, fallback }) {
    settings21.use([settingKey]);
    return /* @__PURE__ */ React.createElement(ColorSettingRow, {
      value: getColor(settingKey, fallback),
      onChange: (v) => {
        settings21.store[settingKey] = v;
        applyColors();
      },
      title,
      description
  ...
  var logger27 = new Logger("ChatStateFavicons");
  ...
  function currentConversationId() {
    ...
    try {
      ...
      logger27.debug("ChatPageStore unavailable:", e);
    }
  ...
  function hasError() {
    ...
    try {
      ...
      logger27.debug("ResponseStore unavailable:", e);
      return false;
    }
  }
  ...
  function attachStores() {
    ...
    try {
      ...
      logger27.debug("RoutingStore subscribe failed:", e);
      try {
        unsubRoute = RoutingStore.useRoutingStore.subscribe(() => scheduleEvaluate());
      } catch (err) {
        logger27.debug("RoutingStore full subscribe failed:", err);
      }
    }
    try {
      ...
      logger27.debug("ChatPageStore subscribe failed:", e);
    }
  }
...
`);
  ...
  function playSound() {
    if (!userGestured)
      return;
    const url = settings25.store.soundUrl?.trim();
    if (url) {
  ...
  var __plugins_default = { [fixChrome_default.name]: fixChrome_default, [noTelemetry_default.name]: noTelemetry_default, [settings_default.name]: settings_default, [chatBarButtons_default.name]: chatBarButtons_default, [contextMenu_default.name]: contextMenu_default, [cloneChats_default.name]: cloneChats_default, [exportChat_default.name]: exportChat_default, [betterFiles_default.name]: betterFiles_default, [settingsFlyout_default.name]: settingsFlyout_default, [incognito_default.name]: incognito_default, [recentTopics_default.name]: recentTopics_default, [noDictation_default.name]: noDictation_default, [messageTimestamps_default.name]: messageTimestamps_default, [downloadTTS_default.name]: downloadTTS_default, [autoCollapse_default.name]: autoCollapse_default, [betterImagine_default.name]: betterImagine_default, [customInstructions_default.name]: customInstructions_default, [placeholder_default.name]: placeholder_default, [autoRetry_default.name]: autoRetry_default, [experiments_default.name]: experiments_default, [noSidebarIdentity_default.name]: noSidebarIdentity_default, [consoleJanitor_default.name]: consoleJanitor_default, [starry_default.name]: starry_default, [inputHistory_default.name]: inputHistory_default, [betterSidebar_default.name]: betterSidebar_default, [usageDisplay_default.name]: usageDisplay_default, [pluginsFlyout_default.name]: pluginsFlyout_default, [noShareLink_default.name]: noShareLink_default, [composerOpacity_default.name]: composerOpacity_default, [noGrokBot_default.name]: noGrokBot_default, [themedScrollbar_default.name]: themedScrollbar_default, [cleaner_default.name]: cleaner_default, [oneko_default.name]: oneko_default, [betterLinks_default.name]: betterLinks_default, [widerChat_default.name]: widerChat_default, [chatStateFavicons_default.name]: chatStateFavicons_default, [streamerMode_default.name]: streamerMode_default, [responseNotification_default.name]: responseNotification_default };
